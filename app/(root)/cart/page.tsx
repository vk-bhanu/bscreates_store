    ...
    <div className='flex gap-20 py-16 px-10 max-lg:flex-col'>
      <div className='w-2/3 max-lg:w-full'>
        ...
        {cart.cartItems.length === 0 ? (
          <p className='text-body-bold'>No Items present in your Cart</p>
        ): (
          <div>
            {cart.cartItems.map((cartItem) =>(
              <div className='flex w-full max-sm:flex-col max-sm:gap-3 hover:bg-grey-1 px-6 py-5 justify-between max-sm:items-start items-center' >
                ...
                <div className='flex gap-4 items-start' >
                  <MinusSquare className="hover:text-red-500 cursor-pointer" onClick={() => cart.decreaseQuantity(cartItem.item._id)} />
                   <p className='text-body-bold'>{cartItem.quantity}</p>
                  <PlusSquare className="hover:text-red-500 cursor-pointer" onClick={() => cart.increaseQuantity(cartItem.item._id)} />
                </div>

                <Trash2 className='hover:text-red-500 cursor-pointer' onClick={() => cart.removeItem(cartItem.item._id)} />
              </div>
            ) )}
          </div>
        ) }
