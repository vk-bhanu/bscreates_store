...
const useCart = create(persist<CartStore>(
    (set, get) => ({
        ...
        addItem: (data: CartItem) => {
            ...
            const isExistingItem = currentItems.find((cartItem) => cartItem.item._id === item._id)
            if(isExistingItem){
                return toast("Item already in Cart")
            }

            set({ cartItems: [...currentItems, { item, quantity, color, size }] })
            toast.success("Item added to your cart", { icon: "🛒" })
        },

        removeItem: (idToRemove: String) => {
            const newCartItems = get().cartItems.filter((carItem)=> carItem.item._id !== idToRemove)
